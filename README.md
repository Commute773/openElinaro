...
- The Discord `/update` command is custom and now runs `git pull --ff-only` in the source workspace immediately.
...
- `bun run service:prepare-update` runs `bun run check`, requires a non-empty human-written change block via `--changes`, `--changes-file`, `OPENELINARO_DEPLOY_CHANGES`, or piped stdin, then computes the next deploy version in `yyyy.mm.dd` or `yyyy.mm.dd.n` form, writes `VERSION.json` plus `DEPLOYMENTS.md` into the source workspace, and commits the current code plus metadata as `update: <version>`.
