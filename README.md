...
- `/update` now syncs the source checkout and formats the pending `DEPLOYMENTS.md` entries newer than the running version; `/update confirm:true` performs the actual managed-service deployment step against the local installation on the host.
