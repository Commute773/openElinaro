...
export async function handleG2ApiRequest(
  ...
  // ── GET /api/g2/tools ──
  if (pathname === "/api/g2/tools" && request.method === "GET") {
    try {
  ...
  // ── POST /api/g2/tools/:name ──
  const toolExecMatch = pathname.match(/^\/api\/g2\/tools\/([^/]+)$/);
  if (toolExecMatch?.[1] && request.method === "POST") {
