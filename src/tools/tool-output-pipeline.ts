...
import { z } from "zod";
import type { AppProgressUpdate } from "../domain/assistant";
...
const pipelineTelemetry = telemetry.child({ component: "tool" });
...
export const TOOL_RESULT_SUMMARY_INPUT_CHAR_LIMIT = 10_000;
...
export async function resolveGuardedToolName(
  toolResults: Pick<ToolResultStore, "get">,
  toolName: string,
  input?: unknown,
) {
...
export async function getUntrustedToolDescriptor(
  toolResults: Pick<ToolResultStore, "get">,
  toolName: string,
  input?: unknown,
): Promise<UntrustedContentDescriptor | undefined> {
  const resolvedToolName = await resolveGuardedToolName(toolResults, toolName, input);
...
export async function normalizeToolResult(
  ...
  const descriptor = toolName ? await getUntrustedToolDescriptor(toolResults, toolName, input) : undefined;
...
export async function finalizeToolResult(
  ...
  return normalizeToolResult(toolResults, result, toolName, input);
}
...
export function buildToolProgressUpdates(name: string, result: unknown) {
  if (name === "openbrowser") {
  ...
  return tool(
    async (input) => {
      ...
      try {
        ...
        return await finalizeToolResult(toolResults, result, entry.name, input);
      } catch (error) {
        return await normalizeToolResult(toolResults, normalizeToolFailure(entry.name, error));
      }
    },
    {
      name: entry.name,
