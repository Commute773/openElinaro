...
import {
  buildToolProgressUpdates,
  finalizeToolResult,
  getToolInputSchema,
  getUntrustedToolDescriptor,
  ...
  notifyToolUse,
  reportProgress,
  resolveGuardedToolName,
  stripToolControlInput,
  TOOL_RESULT_SUMMARY_INPUT_CHAR_LIMIT,
  wrapToolOutput,
...
export class ToolRegistry {
  ...
  private wrapToolWithDefaultCwd(
    ...
    return wrapToolWithDefaultCwd(entry, defaultCwd);
  }

  private wrapToolOutput(entry: StructuredToolInterface, context?: ToolContext): StructuredToolInterface {
    return wrapToolOutput(this.toolResults, entry, (name, input, ctx) => this.injectToolContext(name, input, ctx), context);
  }

  private getToolInputSchema(entry: StructuredToolInterface) {
    return getToolInputSchema(entry);
  }

  private async finalizeToolResult(result: unknown, toolName: string, input?: unknown) {
    return finalizeToolResult(this.toolResults, result, toolName, input);
  }
  ...
  private async normalizeToolResult(result: unknown, toolName?: string, input?: unknown) {
    return normalizeToolResult(this.toolResults, result, toolName, input);
  }

  private normalizeToolFailure(name: string, error: unknown) {
    return normalizeToolFailure(name, error);
  }

  private async getUntrustedToolDescriptor(toolName: string, input?: unknown) {
    return getUntrustedToolDescriptor(this.toolResults, toolName, input);
  }

  private async resolveGuardedToolName(toolName: string, input?: unknown) {
    return resolveGuardedToolName(this.toolResults, toolName, input);
  }
  ...
  private async notifyToolResultProgress(
    ...
    return notifyToolResultProgress(context, name, result, input);
  }

  private async reportProgress(context: ToolContext | undefined, summary: string, input?: unknown) {
    return reportProgress(context, summary, input);
  }
